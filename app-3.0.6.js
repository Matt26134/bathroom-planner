...
(function(){
"use strict";
const VERSION="3.0.6", KEY="bathroomPlannerStable";
...
const starter={
 ...
 project:{name:"Bathroom Redesign",planName:"Current working layout",notes:"Confirm final survey dimensions after strip-out before ordering fitted items.",globalFixtureFinish:"use-item"},
 room:{width:2280,depth:2545,ceiling:2470,tolerance:20,window:{before:660,width:930,after:690,sill:900,height:960,depth:200},door:{before:835,width:860,leafWidth:762,after:850,height:1981,hinge:"bottom",swingDirection:"in",openAngle:26}},
...
if(!["use-item","matt-black","chrome","brushed-brass"].includes(String(state.project.globalFixtureFinish||"use-item"))) state.project.globalFixtureFinish="use-item";
state.room = {...clone(starter.room), ...(state.room||{})};
state.room.window = {...clone(starter.room.window), ...(state.room.window||{})};
if(state.room.window.depth==null) state.room.window.depth=200;
...
if(!Array.isArray(state.pipeRoutes)) state.pipeRoutes=[];
...
function ensureProduct3D(p){
 ...
 // V2.5.7: retain/repair Odesa metadata and force the corrected product-specific renderer.
 ...
 if(sku==="ODAR2X6FSFLCALWTOP"){
  p.style="vanity4drawer";p.drawers=4;p.modules=2;p.drawersPerModule=2;p.visibleDrawerFronts=4;p.drawerConfiguration="2 columns x 2 rows";p.topOptions=["countertop","double-basin"];p.defaultTopStyle=p.defaultTopStyle||"countertop";
...
function ensurePlanVariants(){
 if(!Array.isArray(state.planVariants)||!state.planVariants.length){
  ...
  state.planVariants=[{id,name:state.project?.planName||"Current working layout",items:clone(state.items||[]),services:clone(state.services||[]),pipeRoutes:clone(state.pipeRoutes||[]),createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()}];
  state.activePlanId=id;
 }
 if(!state.activePlanId||!state.planVariants.some(v=>v.id===state.activePlanId))state.activePlanId=state.planVariants[0].id;
 state.planVariants.forEach(v=>{if(!Array.isArray(v.items))v.items=[];if(!Array.isArray(v.services))v.services=[];if(!Array.isArray(v.pipeRoutes))v.pipeRoutes=[];if(!v.name)v.name="Plan";});
}
...
function syncWorkingToActivePlan(){const v=activePlanVariant();if(!v)return;v.name=state.project?.planName||v.name||"Plan";v.items=clone(state.items||[]);v.services=clone(state.services||[]);v.pipeRoutes=clone(state.pipeRoutes||[]);v.updatedAt=new Date().toISOString();}
function loadPlanVariant(id){
 syncWorkingToActivePlan();const v=state.planVariants.find(x=>x.id===id);if(!v)return;
 state.activePlanId=v.id;state.items=clone(v.items||[]);state.services=clone(v.services||[]);state.pipeRoutes=clone(v.pipeRoutes||[]);state.project.planName=v.name||"Plan";
...
ensurePlanVariants();
function save(){syncWorkingToActivePlan();state.version=VERSION;state.schemaVersion=8;state.updatedAt=new Date().toISOString();localStorage.setItem(KEY,JSON.stringify(state))}
...
function render(){
 ...
 if(window.BPPipes?.drawPlanLayer)window.BPPipes.drawPlanLayer(svg,state);
 drawDimensions();
...
function openSelectedSheet(){
 ...
 if(selected.kind==="item"){
  ...
  $("itemFixtureFinishWrap").classList.toggle("hidden",!fixtureTypes.includes(i.type));if(fixtureTypes.includes(i.type)){$("itemFixtureFinish").value=i.finish||defaultFixtureFinish();}
  $("itemVanityTopWrap").classList.toggle("hidden",!isOdesaVanityItem(i));if(isOdesaVanityItem(i)){$("itemVanityTop").value=i.vanityTopStyle||"countertop";}
  ...
  if(isGlass){$("itemStudGlassStyle").value=i.glassStyle||"fluted";$("itemStudGlassTrim").value=i.glassTrimFinish||defaultGlassTrimFinish();$("itemGlassPrivacy").value=i.glassPrivacy||"light";$("itemGlassThickness").value=Number(i.glassThickness)||10;}
  ...
  if(selected.id==="window"){
   ...
   $("openA").value=state.room.window.before;$("openW").value=state.room.window.width;$("openB").value=state.room.window.after;$("openSill").value=state.room.window.sill;$("openHeight").value=state.room.window.height;if($("openDepth"))$("openDepth").value=state.room.window.depth||200;
  }else{
...
function addItem(type){
 ...
 if(type==="rainHead"){i.z=1945;i.height=180;i.finish=defaultFixtureFinish();i.mountHost="left";i.mountAlong=Math.max(160,state.room.depth-405);i.mountFace="shower";syncMountedItemPlan(i)}
 if(type==="handset"){i.z=1050;i.height=720;i.finish=defaultFixtureFinish();i.mountHost="left";i.mountAlong=Math.max(50,state.room.depth-225);i.mountFace="shower";syncMountedItemPlan(i)}
 if(type==="showerControls"){const stud=state.items.find(x=>x.type==="stud");i.z=950;i.height=200;i.finish=defaultFixtureFinish();i.mountHost=stud?"stud:"+stud.id:"left";i.mountAlong=stud?(Number(stud.w)||900)*.65:state.room.depth*.65;i.mountFace="shower";syncMountedItemPlan(i)}
 if(type==="glassPanel"){
  const stud=state.items.find(x=>x.type==="stud");i.glassStyle="fluted";i.glassTrimFinish=defaultGlassTrimFinish();i.glassPrivacy="light";i.glassThickness=10;i.h=10;i.mountHost="free";
...
function sync(){
 ...
 $("windowBefore").value=r.window.before;$("windowWidth").value=r.window.width;$("windowAfter").value=r.window.after;$("windowSill").value=r.window.sill;$("windowHeight").value=r.window.height;if($("windowDepth"))$("windowDepth").value=r.window.depth||200;
 ...
 $("projectName").value=state.project.name||"";$("projectPlanName").value=state.project.planName||"";$("notes").value=state.project.notes||"";if($("globalFixtureFinish"))$("globalFixtureFinish").value=state.project.globalFixtureFinish||"use-item";
...
function placeProduct(id){
 ...
 if(isOdesaProduct(p)) item.vanityTopStyle=p.defaultTopStyle||"countertop";
 if(p.type==="mirror"){
...
async function importProductFile(file){
 try{
  ...
  checkpoint();p.id="prod-"+Date.now();p.builtIn=false;ensureProduct3D(p);state.products.push(p);save();renderProducts();alert("Product imported. V3 uses product configuration metadata and product-specific geometry when available.");
...
function isOdesaProduct(p){const sku=String(p?.sku||"").trim().toUpperCase(),profile=String(p?.render3d?.profile||"").toLowerCase();return sku==="ODAR2X6FSFLCALWTOP"||profile==="odesa-arc-odar2x6fsflcalwtop"}
function isOdesaVanityItem(i){return !!(i&&i.type==="vanity"&&isOdesaProduct(productById(i.productId)))}
function globalFixtureOverride(){const f=String(state?.project?.globalFixtureFinish||"use-item").toLowerCase();return ["matt-black","chrome","brushed-brass"].includes(f)?f:"use-item"}
function defaultFixtureFinish(){const f=globalFixtureOverride();return f==="use-item"?"matt-black":f}
function defaultGlassTrimFinish(){const f=globalFixtureOverride();return f==="use-item"?"matt-black":f}
...
const ITEM_DETAIL_CONTROL_IDS=["itemName","itemType","itemX","itemY","itemW","itemH","itemZ","itemHeight","itemRotation","itemMountHost","itemMountAlong","itemMountFace","itemFixtureFinish","itemVanityTop","itemStudGlassStyle","itemStudGlassTrim","itemGlassPrivacy","itemGlassThickness"];
...
function refreshItemDetailChrome(i,changedId){
 ...
 $("move3DBtn").classList.toggle("hidden",i.locked||(i.type==="niche"&&!i.mountHost));
 ...
 $("itemVanityTopWrap").classList.toggle("hidden",!isOdesaVanityItem(i));
...
function syncPassiveItemDetailFields(i){
 const values={itemName:i.name,itemType:i.type,itemX:Math.round(Number(i.x)||0),itemY:Math.round(Number(i.y)||0),itemW:Math.round(Number(i.w)||0),itemH:Math.round(Number(i.h)||0),itemZ:Math.round(Number(i.z)||0),itemHeight:Math.round(Number(i.height)||0),itemRotation:Math.round((((Number(i.rotation)||0)%360+360)%360)*10)/10,itemMountAlong:Math.round(Number(i.mountAlong)||0),itemFixtureFinish:i.finish||defaultFixtureFinish(),itemVanityTop:i.vanityTopStyle||"countertop",itemStudGlassStyle:i.glassStyle||"fluted",itemStudGlassTrim:i.glassTrimFinish||defaultGlassTrimFinish(),itemGlassPrivacy:i.glassPrivacy||"light",itemGlassThickness:Number(i.glassThickness)||10};
...
function applyItemDetailsLive(changedId){
 ...
 if(["rainHead","handset","showerControls"].includes(i.type))i.finish=$("itemFixtureFinish").value||i.finish||defaultFixtureFinish();
 if(isOdesaVanityItem(i))i.vanityTopStyle=$("itemVanityTop").value||i.vanityTopStyle||"countertop";
 if(i.type==="glassPanel"){
  i.mountHost="free";delete i.mountAlong;delete i.mountFace;i.glassStyle=$("itemStudGlassStyle").value||i.glassStyle||"fluted";i.glassTrimFinish=$("itemStudGlassTrim").value||i.glassTrimFinish||defaultGlassTrimFinish();i.glassPrivacy=$("itemGlassPrivacy").value||i.glassPrivacy||"light";
...
["openA","openW","openB","openSill","openHeight","openDepth"].forEach(id=>$(id)&&($(id).onchange=()=>{checkpoint();state.room.window.before=Math.max(0,Number($("openA").value)||0);state.room.window.width=Math.max(100,Number($("openW").value)||100);state.room.window.after=Math.max(0,Number($("openB").value)||0);state.room.window.sill=Math.max(0,Number($("openSill").value)||0);state.room.window.height=Math.max(100,Number($("openHeight").value)||100);state.room.window.depth=Math.max(20,Number($("openDepth").value)||200);save();sync();render();openSelectedSheet();if(window.BP3DView?.refresh)window.BP3DView.refresh()}));
...
const numPaths={
 ...
 windowBefore:["room","window","before"],windowWidth:["room","window","width"],windowAfter:["room","window","after"],windowSill:["room","window","sill"],windowHeight:["room","window","height"],windowDepth:["room","window","depth"],
...
$("globalFixtureFinish")?.addEventListener("change",e=>{checkpoint();const val=e.target.value||"use-item";state.project.globalFixtureFinish=val;if(val!=="use-item"){state.items.forEach(i=>{if(["rainHead","handset","showerControls"].includes(i.type))i.finish=val;if(i.type==="glassPanel")i.glassTrimFinish=val;if(productById(i.productId)?.type==="tap")i.finish=val;});}save();render();if(selected.kind==="item")openSelectedSheet();if(window.BP3DView?.refresh)window.BP3DView.refresh();});
...
$("duplicatePlanBtn")?.addEventListener("click",()=>{
 ...
 const id="plan-"+Date.now().toString(36),now=new Date().toISOString(),copyPlan={id,name,items:clone(state.items||[]),services:clone(state.services||[]),pipeRoutes:clone(state.pipeRoutes||[]),createdAt:now,updatedAt:now};state.planVariants.push(copyPlan);state.activePlanId=id;state.project.planName=name;save();sync();render();renderPlanVariants();if(window.BP3DView?.refresh)window.BP3DView.refresh();
});
...
svg.addEventListener("pointerdown",e=>{
 if(window.BPPipes?.isDrawing?.())return;
...
window.BP3D={
 ...
 moveWallItem3D:(id,along,z)=>{const i=state.items.find(v=>v.id===id);if(!i||i.locked||!(WALL_MOUNT_TYPES.has(i.type)||(i.type==="niche"&&i.mountHost)))return null;const host=i.mountHost||i.mountWall||"left",stud=hostStud(host),span=stud?Math.max(20,Number(stud.w)||900):((host==="left"||host==="right")?state.room.depth:state.room.width),half=Math.min(span/2,mountPlanSpan(i)/2);i.mountAlong=Math.round(Math.max(half,Math.min(span-half,Number(along)||half)));i.z=Math.round(Math.max(0,Math.min(state.room.ceiling-(Number(i.height)||1),Number(z)||0)));syncMountedItemPlan(i);return {along:i.mountAlong,z:i.z,x:i.x,y:i.y,host};},
 addNiche3D:(cfg={})=>{checkpoint();const width=Math.max(100,Number(cfg.width)||400),depth=Math.max(40,Number(cfg.depth)||90),height=Math.max(100,Number(cfg.height)||300),host=String(cfg.host||"left"),i={id:"niche"+Date.now(),type:"niche",name:"Recess / niche",x:0,y:0,w:width,h:depth,rotation:0,locked:false,z:Math.max(0,Math.min(state.room.ceiling-height,Number(cfg.z)||0)),height,nicheDepth:depth,mountHost:host,mountAlong:Number(cfg.along)||0,mountFace:cfg.mountFace||"shower"};syncMountedItemPlan(i);state.items.push(i);selected={kind:"item",id:i.id};save();render();openSelectedSheet();window.BP3DView?.refresh?.();return clone(i);},
 clone:(v)=>clone(v),
