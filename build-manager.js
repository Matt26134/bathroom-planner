(function(){
  ...
  function ensure(){
    ...
    if(!Array.isArray(s.structure.noggins))s.structure.noggins=[];
  ...
  function renderStructure(){
    ...
    $("joistDirection").value=st.direction;$("joistSpacing").value=st.spacing;$("joistWidth").value=st.width;$("joistDepth").value=st.depth;$("joistOffset").value=st.offset;$("deckThickness").value=st.deckThickness;
    const js=generatedJoists();$("joistSummary").innerHTML=`<strong>${js.length} generated joists</strong> · ${st.width} × ${st.depth} mm · ${st.spacing} mm centres.<br>Floor void reference depth: approximately ${st.depth} mm beneath the structural deck.`;
  ...
  function exportBathplan(){const s=clone(state());s.version=api.version||s.version;s.schemaVersion=3;s.updatedAt=new Date().toISOString();const pack={bathplanFormat:1,appVersion:api.version||"2.2.0",exportedAt:new Date().toISOString(),assetsEmbedded:true,project:s};download(`${safeName(s.project?.name)}-${new Date().toISOString().slice(0,10)}.bathplan`,JSON.stringify(pack));}
