...
if (!api || !legacy) {
  ...
  status.textContent = "Visual 3D · renderer V2.5.8 · tap a fixture to edit";
  window.BP3D_RENDERER_VERSION = "2.5.8";
  ...
  function buildTrentTap(i,s,prod){
    const g=groupForItem(i),w=mm(i.w||200),d=mm(i.h||336),h=mm(i.height||1165),z=mm(i.z||0),mat=productMetalFinishMaterial(i,s,prod,"chrome");
  ...
  function buildArezzoVanity(i,s,prod){
    ...
    const drain=cylinder(.014,.006,mats.chrome,"y",22);drain.position.set(0,z+bodyH+basinH*.64,.034);g.add(drain);
  ...
  function buildOdesaArc1200Vanity(i,s,prod){
    ...
    if(topStyle==="double-basin"){
      const basinH=Math.max(.055,Math.min(.075,h*.085)),topY=z+plinthH+bodyH;
      g.add(roundedFootprintBox(w+.014,basinH,d+.016,arcR+.010,mats.ceramic,0,topY,0));
      g.add(roundedBox(w*.78,.010,.016,.004,mats.ceramic,0,topY+basinH*.92,-d*.34,false));
      const bowlW=Math.max(.25,w*.315),bowlD=Math.max(.16,d*.44),bowlH=Math.max(.020,basinH*.50),tapMat=productMetalFinishMaterial(i,s,prod,"brushed-brass");
      [-w*.24,w*.24].forEach(cx=>{
        g.add(roundedBox(bowlW,bowlH,bowlD,.028,mats.ceramicInner,cx,topY+basinH*.46,.018,false));
        const drain=cylinder(.014,.006,mats.chrome,"y",22);drain.position.set(cx,topY+basinH*.60,.020);g.add(drain);
        const stem=cylinder(.010,.120,tapMat,"y",18);stem.position.set(cx,topY+basinH*.95,-d*.20);g.add(stem);
        const spout=tubePath([new THREE.Vector3(cx,topY+basinH*.98,-d*.20),new THREE.Vector3(cx,topY+basinH*1.12,-d*.18),new THREE.Vector3(cx,topY+basinH*1.12,-d*.08)],.010,tapMat,22);g.add(spout);
        const lever=meshBox(.013,.010,.050,tapMat,cx+.018,topY+basinH*1.07,-d*.21,false);g.add(lever);
      });
    }else{
