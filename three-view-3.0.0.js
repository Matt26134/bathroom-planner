...
if (!api || !legacy) {
  ...
  status.textContent = "Visual 3D · renderer V3.0.0 · tap a fixture to edit";
  window.BP3D_RENDERER_VERSION = "3.0.0";
  ...
  scene.add(root, wallRoot, surfaceRoot, structureRoot, itemRoot, fixedRoot, measureRoot);
  ...
  function updateWallVisibility() {
    ...
    root.visible=!xray;structureRoot.visible=xray;itemRoot.visible=true;fixedRoot.visible=true;measureRoot.visible=true;
  ...
  function saveCurrentCamera(){const s=api.getState();s.ui=s.ui||{};s.ui.savedCamera={position:{x:camera.position.x,y:camera.position.y,z:camera.position.z},target:{x:target.x,y:target.y,z:target.z},savedAt:new Date().toISOString()};api.persist?.();status.textContent="Camera view saved";setTimeout(()=>status.textContent="Visual 3D · renderer V3.0.0 · tap a fixture to edit",1400);}
  ...
  function toggleMeasure3D(){measureMode3D=!measureMode3D;clear3DMeasure();measure3DBtn?.classList.toggle("active3d",measureMode3D);measure3DBtn?.setAttribute("aria-pressed",measureMode3D?"true":"false");status.textContent=measureMode3D?"3D measure · tap first surface":"Visual 3D · renderer V3.0.0 · tap a fixture to edit";}
  function applySection(mode){sectionMode=sectionMode===mode?null:mode;const s=api.getState(),W=mm(s.room.width),D=mm(s.room.depth);renderer.clippingPlanes=[];if(sectionMode==="x")renderer.clippingPlanes=[new THREE.Plane(new THREE.Vector3(-1,0,0),W*.52)];if(sectionMode==="y")renderer.clippingPlanes=[new THREE.Plane(new THREE.Vector3(0,0,-1),D*.52)];sectionXBtn?.classList.toggle("active3d",sectionMode==="x");sectionYBtn?.classList.toggle("active3d",sectionMode==="y");sectionXBtn?.setAttribute("aria-pressed",sectionMode==="x"?"true":"false");sectionYBtn?.setAttribute("aria-pressed",sectionMode==="y"?"true":"false");status.textContent=sectionMode?`Section ${sectionMode.toUpperCase()} · cut at room centre`:"Visual 3D · renderer V3.0.0 · tap a fixture to edit";}
