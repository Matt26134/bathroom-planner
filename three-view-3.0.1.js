...
if (!api || !legacy) {
  ...
  status.textContent = "Visual 3D · renderer V3.0.1 · tap a fixture to edit";
  window.BP3D_RENDERER_VERSION = "3.0.1";
  ...
  function pipePointZ(route,index){
    const pts=route.points||[],p=pts[index];if(Number.isFinite(Number(p?.z)))return Number(p.z);
    let total=0,run=0;for(let n=1;n<pts.length;n++)total+=Math.hypot((pts[n].x||0)-(pts[n-1].x||0),(pts[n].y||0)-(pts[n-1].y||0));
    for(let n=1;n<=index&&n<pts.length;n++)run+=Math.hypot((pts[n].x||0)-(pts[n-1].x||0),(pts[n].y||0)-(pts[n-1].y||0));
    const a=Number(route.zStart)||0,b=Number.isFinite(Number(route.zEnd))?Number(route.zEnd):a;return total>0?a+(b-a)*(run/total):a;
  }
  function buildPipeRoutes(s){
    const colors={hot:0xd84a3d,cold:0x2878d8,waste:0x159a7f,soil:0x363a3a};
    (s.pipeRoutes||[]).filter(r=>r.visible!==false&&(r.points||[]).length>1).forEach(route=>{
      ...
      const radius=Math.max(.006,Math.min(.055,mm(route.diameter||15)/2));
      const mat=new THREE.MeshStandardMaterial({color:colors[route.type]||0x777777,roughness:.42,metalness:.04});mat.userData.bpOwned=true;
      for(let i=1;i<pts.length;i++){const a=pts[i-1],b=pts[i],dir=b.clone().sub(a),len=dir.length();if(len<.001)continue;const geo=new THREE.CylinderGeometry(radius,radius,len,12),seg=new THREE.Mesh(geo,mat);seg.position.copy(a).add(b).multiplyScalar(.5);seg.quaternion.setFromUnitVectors(new THREE.Vector3(0,1,0),dir.normalize());seg.castShadow=true;seg.receiveShadow=true;seg.userData.pipeRouteId=route.id;pipeRoot.add(seg);}
      pts.forEach(pt=>{const joint=new THREE.Mesh(new THREE.SphereGeometry(radius*1.18,14,10),mat);joint.position.copy(pt);joint.userData.pipeRouteId=route.id;pipeRoot.add(joint)});
    });
  }
  ...
  function updateWallVisibility() {
    if(!room)return;
    const W=mm(room.width),D=mm(room.depth),mode=currentWallMode(),show=mode!=="none",xray=floorXrayToggle?.checked===true;
    root.visible=!xray;structureRoot.visible=xray;itemRoot.visible=true;fixedRoot.visible=true;pipeRoot.visible=true;measureRoot.visible=true;
    structureRoot.children.forEach(m=>{if(m.userData?.structureKind==="deck"||m.userData?.structureKind==="heating")m.visible=!xray;else m.visible=xray;});
  ...
  function saveCurrentCamera(){const s=api.getState();s.ui=s.ui||{};s.ui.savedCamera={position:{x:camera.position.x,y:camera.position.y,z:camera.position.z},target:{x:target.x,y:target.y,z:target.z},savedAt:new Date().toISOString()};api.persist?.();status.textContent="Camera view saved";setTimeout(()=>status.textContent="Visual 3D · renderer V3.0.1 · tap a fixture to edit",1400);}
  ...
  function toggleMeasure3D(){measureMode3D=!measureMode3D;clear3DMeasure();measure3DBtn?.classList.toggle("active3d",measureMode3D);measure3DBtn?.setAttribute("aria-pressed",measureMode3D?"true":"false");status.textContent=measureMode3D?"3D measure · tap first surface":"Visual 3D · renderer V3.0.1 · tap a fixture to edit";}
  function applySection(mode){sectionMode=sectionMode===mode?null:mode;const s=api.getState(),W=mm(s.room.width),D=mm(s.room.depth);renderer.clippingPlanes=[];if(sectionMode==="x")renderer.clippingPlanes=[new THREE.Plane(new THREE.Vector3(-1,0,0),W*.52)];if(sectionMode==="y")renderer.clippingPlanes=[new THREE.Plane(new THREE.Vector3(0,0,-1),D*.52)];sectionXBtn?.classList.toggle("active3d",sectionMode==="x");sectionYBtn?.classList.toggle("active3d",sectionMode==="y");sectionXBtn?.setAttribute("aria-pressed",sectionMode==="x"?"true":"false");sectionYBtn?.setAttribute("aria-pressed",sectionMode==="y"?"true":"false");status.textContent=sectionMode?`Section ${sectionMode.toUpperCase()} · cut at room centre`:"Visual 3D · renderer V3.0.1 · tap a fixture to edit";}
