...
if (!api || !legacy) {
  ...
  status.textContent = "Visual 3D · renderer V3.0.2 · tap a fixture to edit";
  window.BP3D_RENDERER_VERSION = "3.0.2";
  ...
  function buildPipeRoutes(s){
    const colors={hot:0xd84a3d,cold:0x2878d8,mixed:0x8a54d6,waste:0x159a7f,soil:0x363a3a},focus=plumbingFocusActive();
    (s.pipeRoutes||[]).filter(r=>r.visible!==false&&(r.points||[]).length>1).forEach(route=>{
      const pts=(route.points||[]).map((p,i)=>new THREE.Vector3(mm(p.x),mm(pipePointZ(route,i)),mm(p.y)));
      const radius=Math.max(.006,Math.min(.055,mm(route.diameter||15)/2))*(focus?1.16:1);
      const col=colors[route.type]||0x777777,mat=new THREE.MeshStandardMaterial({color:col,roughness:focus?.24:.42,metalness:.04,emissive:focus?col:0x000000,emissiveIntensity:focus?.20:0});mat.userData.bpOwned=true;
      for(let i=1;i<pts.length;i++){const a=pts[i-1],b=pts[i],dir=b.clone().sub(a),len=dir.length();if(len<.001)continue;const geo=new THREE.CylinderGeometry(radius,radius,len,focus?18:12),seg=new THREE.Mesh(geo,mat);seg.position.copy(a).add(b).multiplyScalar(.5);seg.quaternion.setFromUnitVectors(new THREE.Vector3(0,1,0),dir.normalize());seg.castShadow=!focus;seg.receiveShadow=true;seg.userData.pipeRouteId=route.id;pipeRoot.add(seg);}
      pts.forEach(pt=>{const joint=new THREE.Mesh(new THREE.SphereGeometry(radius*1.18,focus?18:14,focus?14:10),mat);joint.position.copy(pt);joint.userData.pipeRouteId=route.id;pipeRoot.add(joint)});
    });
  }
  ...
  function setPlumbingFocus(on){const s=api.getState();s.ui=s.ui||{};s.ui.plumbingFocus=!!on;api.persist?.();rebuildScene(true);updateWallVisibility();status.textContent=on?'Plumbing focus · fixtures ghosted · joists solid · pipes prioritised':'Visual 3D · renderer V3.0.2 · tap a fixture to edit';}
  ...
  function saveCurrentCamera(){const s=api.getState();s.ui=s.ui||{};s.ui.savedCamera={position:{x:camera.position.x,y:camera.position.y,z:camera.position.z},target:{x:target.x,y:target.y,z:target.z},savedAt:new Date().toISOString()};api.persist?.();status.textContent="Camera view saved";setTimeout(()=>status.textContent="Visual 3D · renderer V3.0.2 · tap a fixture to edit",1400);}
  ...
  function toggleMeasure3D(){measureMode3D=!measureMode3D;clear3DMeasure();measure3DBtn?.classList.toggle("active3d",measureMode3D);measure3DBtn?.setAttribute("aria-pressed",measureMode3D?"true":"false");status.textContent=measureMode3D?"3D measure · tap first surface":"Visual 3D · renderer V3.0.2 · tap a fixture to edit";}
  function applySection(mode){sectionMode=sectionMode===mode?null:mode;const s=api.getState(),W=mm(s.room.width),D=mm(s.room.depth);renderer.clippingPlanes=[];if(sectionMode==="x")renderer.clippingPlanes=[new THREE.Plane(new THREE.Vector3(-1,0,0),W*.52)];if(sectionMode==="y")renderer.clippingPlanes=[new THREE.Plane(new THREE.Vector3(0,0,-1),D*.52)];sectionXBtn?.classList.toggle("active3d",sectionMode==="x");sectionYBtn?.classList.toggle("active3d",sectionMode==="y");sectionXBtn?.setAttribute("aria-pressed",sectionMode==="x"?"true":"false");sectionYBtn?.setAttribute("aria-pressed",sectionMode==="y"?"true":"false");status.textContent=sectionMode?`Section ${sectionMode.toUpperCase()} · cut at room centre`:"Visual 3D · renderer V3.0.2 · tap a fixture to edit";}
  ...
  function endPointer(e){
    ...
    if(was&&!was.moved&&!moveMode){if(measureMode3D)measureAt(e.clientX,e.clientY);else selectAt(e.clientX,e.clientY);}
    if(was)drag=null;
  }
  ...
  function setFloorXray(on, moveCamera=true){
    ...
    status.textContent=on?"Floor removed · fixtures stay visible · joists exposed":(plumbingFocusActive()?"Plumbing focus · fixtures ghosted · joists solid · pipes prioritised":"Visual 3D · renderer V3.0.2 · tap a fixture to edit");
  }
