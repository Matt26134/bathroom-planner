...
if (!api || !legacy) {
  ...
  status.textContent = "Visual 3D · renderer V3.0.4 · tap a fixture or pipe to edit";
  window.BP3D_RENDERER_VERSION = "3.0.4";
  ...
  function setPlumbingFocus(on){const s=api.getState();s.ui=s.ui||{};s.ui.plumbingFocus=!!on;api.persist?.();rebuildScene(true);updateWallVisibility();status.textContent=on?'Plumbing focus · fixtures ghosted · joists solid · pipes prioritised':'Visual 3D · renderer V3.0.4 · tap a fixture or pipe to edit';}
  ...
  function saveCurrentCamera(){const s=api.getState();s.ui=s.ui||{};s.ui.savedCamera={position:{x:camera.position.x,y:camera.position.y,z:camera.position.z},target:{x:target.x,y:target.y,z:target.z},savedAt:new Date().toISOString()};api.persist?.();status.textContent="Camera view saved";setTimeout(()=>status.textContent="Visual 3D · renderer V3.0.4 · tap a fixture or pipe to edit",1400);}
  ...
  function toggleMeasure3D(){measureMode3D=!measureMode3D;clear3DMeasure();measure3DBtn?.classList.toggle("active3d",measureMode3D);measure3DBtn?.setAttribute("aria-pressed",measureMode3D?"true":"false");status.textContent=measureMode3D?"3D measure · tap first surface":"Visual 3D · renderer V3.0.4 · tap a fixture or pipe to edit";}
  function applySection(mode){sectionMode=sectionMode===mode?null:mode;const s=api.getState(),W=mm(s.room.width),D=mm(s.room.depth);renderer.clippingPlanes=[];if(sectionMode==="x")renderer.clippingPlanes=[new THREE.Plane(new THREE.Vector3(-1,0,0),W*.52)];if(sectionMode==="y")renderer.clippingPlanes=[new THREE.Plane(new THREE.Vector3(0,0,-1),D*.52)];sectionXBtn?.classList.toggle("active3d",sectionMode==="x");sectionYBtn?.classList.toggle("active3d",sectionMode==="y");sectionXBtn?.setAttribute("aria-pressed",sectionMode==="x"?"true":"false");sectionYBtn?.setAttribute("aria-pressed",sectionMode==="y"?"true":"false");status.textContent=sectionMode?`Section ${sectionMode.toUpperCase()} · cut at room centre`:"Visual 3D · renderer V3.0.4 · tap a fixture or pipe to edit";}
  ...
  window.BP3DView={startMove:startMoveMode,stopMove:()=>stopMoveMode(true),refresh:()=>rebuildScene(true),setWallMode,setFidelity:(m)=>applyFidelity(m,true),preset,saveCamera:saveCurrentCamera,section:applySection,toggleMeasure:toggleMeasure3D};
  ...
  function cancelNiche3DPlace(){nichePlaceMode=false;updateNiche3DOverlay();status.textContent="Visual 3D · renderer V3.0.4 · tap a fixture or pipe to edit";}
  function placeNiche3DAt(clientX,clientY){
    if(!nichePlaceMode)return false;
    rayFromClient(clientX,clientY);
    const s=api.getState(),studGroups=itemRoot.children.filter(g=>{const i=s.items?.find(x=>x.id===g.userData?.itemId);return i?.type==="stud"}),hits=raycaster.intersectObjects([...studGroups,...wallRoot.children],true).filter(h=>h.object.visible!==false);
    const hit=hits[0];if(!hit){status.textContent="Place recess · tap a solid room wall or stud face";return true;}
    ...
    const hitItemId=hit.object.userData?.itemId,stud=hitItemId?s.items?.find(x=>x.id===hitItemId&&x.type==="stud"):null;
    if(stud){
      ...
      host=hit.object.userData?.wallSide||null;if(!host){status.textContent="Place recess · choose the room wall itself rather than a fixture";return true;}along=(host==="left"||host==="right")?p.z*1000:p.x*1000;
    }
  ...
  function selectAt(clientX,clientY) {
    ...
    if(pipeHit){window.BPPipes?.selectRoute?.(pipeHit.object.userData.pipeRouteId);rebuildScene(true);status.textContent="Pipe selected · use the height controls to move the complete route";return;}
    ...
    if(hit){
      ...
      setTimeout(()=>status.textContent="Visual 3D · renderer V3.0.4 · tap a fixture or pipe to edit",1800);
    }
  }
  function addPipe3DAt(clientX,clientY){
    if(!window.BPPipes?.isDrawing3D?.())return false;
    rayFromClient(clientX,clientY);
    const candidates=[...wallRoot.children,...surfaceRoot.children,...structureRoot.children,...root.children,...itemRoot.children];
    const hits=raycaster.intersectObjects(candidates,true).filter(h=>h.object.visible!==false);
    const hit=hits[0];if(!hit){status.textContent="3D pipe draw · tap a room, floor, wall, stud or fixture surface";return true;}
    const p=hit.point,added=window.BPPipes.add3DPoint?.({x:p.x*1000,y:p.z*1000,z:p.y*1000});
    if(added){rebuildScene(true);const mode=window.BPPipes.get3DMode?.()||"level";status.textContent=`3D pipe draw · point added · ${mode} next`;}
    return true;
  }
  ...
  function setFloorXray(on, moveCamera=true){
    ...
    status.textContent=on?"Floor removed · fixtures stay visible · joists exposed":(plumbingFocusActive()?"Plumbing focus · fixtures ghosted · joists solid · pipes prioritised":"Visual 3D · renderer V3.0.4 · tap a fixture or pipe to edit");
  }
  ...
  sectionXBtn?.addEventListener("click",()=>applySection("x"));sectionYBtn?.addEventListener("click",()=>applySection("y"));
