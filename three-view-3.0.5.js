...
const fidelityModeSelect = document.getElementById("fidelityMode");
const cameraPresetSelect = document.getElementById("cameraPreset");
const saveCameraBtn = document.getElementById("saveCameraBtn");
const fullScreen3DBtn = document.getElementById("fullScreen3DBtn");
const measure3DBtn = document.getElementById("measure3DBtn");
const sectionXBtn = document.getElementById("sectionXBtn");
const sectionYBtn = document.getElementById("sectionYBtn");
const plumbingFocusBtn = document.getElementById("plumbingFocusBtn");
const add3DBtn = document.getElementById("add3DBtn");
const add3DMenu = document.getElementById("add3DMenu");
const add3DNicheBtn = document.getElementById("add3DNicheBtn");
const niche3DPlaceBar = document.getElementById("niche3DPlaceBar");
const niche3DCancelBtn = document.getElementById("niche3DCancelBtn");
const niche3DWidth = document.getElementById("niche3DWidth");
const niche3DHeight = document.getElementById("niche3DHeight");
const niche3DDepth = document.getElementById("niche3DDepth");
...
if (!api || !legacy) {
  ...
  status.textContent = "Visual 3D · renderer V3.0.5 · tap a fixture or pipe to edit";
  window.BP3D_RENDERER_VERSION = "3.0.5";
  ...
  scene.add(root, wallRoot, surfaceRoot, structureRoot, itemRoot, fixedRoot, pipeRoot, measureRoot);
  ...
  const mats = {
    ...
    ceramic:new THREE.MeshPhysicalMaterial({color:0xf8f8f5,roughness:.18,clearcoat:.28,clearcoatRoughness:.18}),ceramicInner:new THREE.MeshPhysicalMaterial({color:0xe7e8e5,roughness:.34,clearcoat:.12}),ceramicWarm:new THREE.MeshPhysicalMaterial({color:0xf4efe6,roughness:.22,clearcoat:.24,clearcoatRoughness:.20}),ceramicWarmInner:new THREE.MeshPhysicalMaterial({color:0xe9e2d8,roughness:.38,clearcoat:.10}),ceramicMatt:new THREE.MeshStandardMaterial({color:0xf4f3ef,roughness:.58}),ceramicMattInner:new THREE.MeshStandardMaterial({color:0xe5e4df,roughness:.70}),acrylic:new THREE.MeshPhysicalMaterial({color:0xfbfaf7,roughness:.22,clearcoat:.34,clearcoatRoughness:.15}),
  ...
  let sceneInitialised=false, measureMode3D=false, measurePoints=[], sectionMode=null, nichePlaceMode=false;
  ...
  function finishPalette(s){return s?.project?.finishPalette||{metal:globalFinishName(s)||"brushed-brass",sanitary:"bright-white",furniture:"product",glass:"fluted",glassTrim:"inherit-metal"}}
  function sanitaryMaterial(s,inner=false){const f=String(finishPalette(s).sanitary||"bright-white").toLowerCase();if(f==="warm-white")return inner?mats.ceramicWarmInner:mats.ceramicWarm;if(f==="soft-matt-white")return inner?mats.ceramicMattInner:mats.ceramicMatt;return inner?mats.ceramicInner:mats.ceramic}
  function furnitureMaterial(prod,s,fallback=mats.timber){const f=String(finishPalette(s).furniture||"product").toLowerCase();if(f==="california-walnut")return mats.californiaWalnut;if(f==="light-oak")return mats.apexOak;if(f==="dark-walnut")return mats.walnut;if(f==="white")return sanitaryMaterial(s,false);return finishMaterial(prod,fallback)}
  function detailLevel(s){return String(s?.ui?.fidelity||"detailed").toLowerCase()}
  function detailAtLeast(s,level){const n={draft:0,detailed:1,presentation:2};return (n[detailLevel(s)]??1)>=(n[level]??1)}
  ...
  function buildNewhamBath(i,s,prod){
    const g=groupForItem(i),w=mm(i.w||1520),d=mm(i.h||730),h=mm(i.height||550),z=mm(i.z||0),outerMat=sanitaryMaterial(s,false),innerMat=sanitaryMaterial(s,true);
    ...
    const metal=accessoryMaterial(i,s,mats.chrome,prod),drain=cylinder(.022,.008,metal,"y",36);drain.position.set(0,z+h*.185,0);g.add(drain);
    const overflow=cylinder(.014,.009,metal,"z",30);overflow.position.set(0,z+h*.63,-d*.36);g.add(overflow);
  ...
  function buildMertonBath(i,s,prod){const g=groupForItem(i),w=mm(i.w),d=mm(i.h),h=mm(i.height||585),z=mm(i.z||0),hand=productProfile(prod).includes("right")?"right":"left",outer=curvedCornerShape(w,d,hand,0),inner=curvedCornerShape(w,d,hand,.062),cer=sanitaryMaterial(s,false),inn=sanitaryMaterial(s,true);outer.holes.push(inner);const geo=new THREE.ExtrudeGeometry(outer,{depth:h*.94,steps:1,bevelEnabled:true,bevelSegments:4,bevelSize:.012,bevelThickness:.010});geo.rotateX(-Math.PI/2);geo.translate(0,z,0);const shell=new THREE.Mesh(geo,cer);shell.castShadow=true;shell.receiveShadow=true;g.add(shell);const floorGeo=new THREE.ShapeGeometry(curvedCornerShape(w,d,hand,.105),40);floorGeo.rotateX(-Math.PI/2);floorGeo.translate(0,z+h*.24,0);g.add(new THREE.Mesh(floorGeo,inn));const drain=cylinder(.019,.009,accessoryMaterial(i,s,mats.chrome,prod),"y",30);drain.position.set(hand==="left"?w*.20:-w*.20,z+h*.255,0);g.add(drain);const overflow=cylinder(.013,.008,accessoryMaterial(i,s,mats.chrome,prod),"z",24);overflow.position.set(hand==="left"?w*.38:-w*.38,z+h*.62,-d*.36);g.add(overflow);return setItemId(g,i.id)}
  function buildBath(i,s){
    const prod=(s.products||[]).find(p=>p.id===i.productId),profile=productProfile(prod);
    if(profile==="newham-c51092")return buildNewhamBath(i,s,prod);
    if(profile==="merton-curved-left"||profile==="merton-curved-right")return buildMertonBath(i,s,prod);
    const g=groupForItem(i),w=mm(i.w||1600),d=mm(i.h||700),h=mm(i.height||550),z=mm(i.z||0),outer=sanitaryMaterial(s,false),inner=sanitaryMaterial(s,true),metal=accessoryMaterial(i,s,mats.chrome,prod);
    const cfg=String(i.v3?.family||"auto"),style=String(prod?.style||"").toLowerCase();
    const family=cfg!=="auto"?cfg:(style.includes("double")?"double-ended":style.includes("back")?"back-to-wall":style.includes("free")?"freestanding":"single-ended");
    const outerR=family==="back-to-wall"?Math.min(.045,d*.08):Math.min(d*.22,w*.09),wall=.055;
    const outerShape=roundedRectShape(w*.985,d*.985,outerR),innerShape=roundedRectShape(w*.985-wall*2,d*.985-wall*2,Math.max(.035,outerR-wall*.25));outerShape.holes.push(innerShape);
    const shellGeo=new THREE.ExtrudeGeometry(outerShape,{depth:h*.80,steps:1,bevelEnabled:true,bevelSegments:detailAtLeast(s,"presentation")?5:3,bevelSize:.012,bevelThickness:.010});shellGeo.rotateX(-Math.PI/2);shellGeo.translate(0,z+.035,0);const shell=new THREE.Mesh(shellGeo,outer);shell.castShadow=true;shell.receiveShadow=true;g.add(shell);
    const rim=roundedRing(w*.99,d*.99,.050,.026,outer,outerR);rim.position.y=z+h*.80;g.add(rim);
    // Deep inner floor and long side slopes make the bath read as a real vessel rather than a white box.
    const floorW=w*.76,floorD=d*.57;g.add(roundedFootprintBox(floorW,.020,floorD,Math.min(.10,floorD*.22),inner,0,z+h*.16,0,false));
    const sideH=h*.49,sideD=.024;const side1=roundedBox(floorW,sideH,sideD,.012,inner,0,z+h*.43,-floorD*.51,false),side2=roundedBox(floorW,sideH,sideD,.012,inner,0,z+h*.43,floorD*.51,false);side1.rotation.x=-.20;side2.rotation.x=.20;g.add(side1,side2);
    const endD=floorD*.96,endH=h*.48,leftEnd=roundedBox(.032,endH,endD,.018,inner,-floorW*.50,z+h*.43,0,false),rightEnd=roundedBox(.032,endH,endD,.018,inner,floorW*.50,z+h*.43,0,false);
    if(family==="double-ended"||family==="freestanding"){leftEnd.rotation.z=-.30;rightEnd.rotation.z=.30}else{leftEnd.rotation.z=-.34;rightEnd.rotation.z=.09}g.add(leftEnd,rightEnd);
    if(family==="back-to-wall"){g.add(roundedBox(w*.95,h*.72,.028,.010,outer,0,z+h*.39,-d*.485,false));}
    // Waste, plug, overflow and subtle overflow bezel.
    const drainX=family==="single-ended"?w*.22:0,drain=cylinder(.021,.008,metal,"y",32);drain.position.set(drainX,z+h*.178,0);g.add(drain);
    const plugRing=new THREE.Mesh(new THREE.TorusGeometry(.027,.0028,8,30),metal);plugRing.rotation.x=Math.PI/2;plugRing.position.set(drainX,z+h*.185,0);g.add(plugRing);
    const overflow=cylinder(.014,.009,metal,"z",28);overflow.position.set(family==="single-ended"?-w*.35:0,z+h*.59,-d*.365);g.add(overflow);
    if(detailAtLeast(s,"presentation")){const shadow=roundedFootprintBox(w*.88,.010,d*.76,Math.min(.08,d*.17),new THREE.MeshStandardMaterial({color:0x8d8d88,transparent:true,opacity:.13,roughness:1}),0,z+.012,0,false);g.add(shadow)}
    return setItemId(g,i.id);
  }

  function buildMetroWC(i,s,prod){
    const g=groupForItem(i),w=mm(i.w||380),d=mm(i.h||650),h=mm(i.height||820),z=mm(i.z||0),cer=sanitaryMaterial(s,false),inn=sanitaryMaterial(s,true),metal=accessoryMaterial(i,s,mats.chrome,prod),panH=mm(prod?.panHeightMm||415),cisH=Math.max(.25,h-panH*.96),cisD=d*.235,cisZ=-d*.365;
    // Compact close-coupled cistern with a genuinely separate lid and flush control.
    g.add(roundedBox(w*.91,cisH,cisD,.042,cer,0,z+panH+cisH*.50,cisZ));g.add(roundedBox(w*.93,.028,cisD*1.045,.035,cer,0,z+panH+cisH+.004,cisZ,false));const flush=cylinder(.018,.007,metal,"y",30);flush.position.set(0,z+panH+cisH+.022,cisZ);g.add(flush);const flushRing=new THREE.Mesh(new THREE.TorusGeometry(.022,.0022,8,28),metal);flushRing.rotation.x=Math.PI/2;flushRing.position.copy(flush.position);g.add(flushRing);
    // Rear neck and sculpted pan.
    g.add(roundedBox(w*.50,panH*.72,d*.32,.052,cer,0,z+panH*.35,-d*.18));const bowl=new THREE.Mesh(new THREE.SphereGeometry(.5,52,34),cer);bowl.scale.set(w*.80,panH*.30,d*.61);bowl.position.set(0,z+panH*.65,d*.105);bowl.castShadow=true;bowl.receiveShadow=true;g.add(bowl);g.add(roundedBox(w*.74,panH*.16,d*.46,.090,cer,0,z+panH*.70,-d*.015));
    // Visible bowl recess, seat ring, lid and hinges.
    const recess=new THREE.Mesh(new THREE.SphereGeometry(.5,44,28),inn);recess.scale.set(w*.55,panH*.085,d*.36);recess.position.set(0,z+panH*.90,d*.09);g.add(recess);const seat=roundedRing(w*.76,d*.48,.043,.023,inn,.14);seat.position.set(0,z+panH*.94,d*.083);g.add(seat);const lid=roundedBox(w*.75,.017,d*.46,.13,cer,0,z+panH*.982,d*.072,false);lid.rotation.x=-.020;g.add(lid);if(detailAtLeast(s,"detailed"))[-.12,.12].forEach(x=>{const hinge=cylinder(.009,.034,metal,"x",18);hinge.position.set(x,z+panH*.988,-d*.12);g.add(hinge)});
    // Floor footprint shadow / fixing line prevents the pan looking like it floats.
    g.add(roundedFootprintBox(w*.40,.010,d*.30,.045,new THREE.MeshStandardMaterial({color:0x777,transparent:true,opacity:.10,roughness:1}),0,z+.006,-d*.05,false));
    return setItemId(g,i.id)
  }

  function buildWC(i,s){
    const prod=(s.products||[]).find(p=>p.id===i.productId),profile=productProfile(prod);if(profile==="metro-metcc")return buildMetroWC(i,s,prod);
    const g=groupForItem(i),w=mm(i.w||370),d=mm(i.h||640),h=mm(i.height||800),z=mm(i.z||0),cer=sanitaryMaterial(s,false),inn=sanitaryMaterial(s,true),metal=accessoryMaterial(i,s,mats.chrome,prod);
    const cfg=String(i.v3?.family||"auto"),style=String(prod?.style||"").toLowerCase();
    const family=cfg!=="auto"?cfg:(style.includes("hung")?"wall-hung":style.includes("back")?"back-to-wall":"close-coupled");
    const seatY=z+Math.min(.44,h*.53),panD=d*.67;
    if(family==="close-coupled"){
      const cisH=h*.43,cisD=d*.235,cisZ=-d*.35;g.add(roundedBox(w*.88,cisH,cisD,.035,cer,0,z+h*.73,cisZ));g.add(roundedBox(w*.90,.028,cisD*1.05,.025,cer,0,z+h*.955,cisZ,false));
      const flush=cylinder(.017,.007,metal,"y",28);flush.position.set(0,z+h*.975,cisZ);g.add(flush);
    }else if(family==="back-to-wall"){
      g.add(roundedBox(w*.78,h*.46,d*.15,.025,cer,0,z+h*.69,-d*.405));const flush=cylinder(.016,.007,metal,"z",28);flush.position.set(0,z+h*.76,-d*.49);g.add(flush);
    }else{
      const plate=roundedBox(w*.42,.105,.010,.012,accessoryMaterial(i,s,mats.chrome,prod),0,z+h*.70,-d*.495,false);g.add(plate);[-.09,.09].forEach(x=>{const b=cylinder(.018,.006,metal,"z",22);b.position.set(x,z+h*.70,-d*.505);g.add(b)});
    }
    // Pan body: tapered pedestal/neck plus a rounded forward bowl.
    if(family!=="wall-hung")g.add(roundedBox(w*.46,seatY-z-.06,d*.36,.055,cer,0,z+(seatY-z)*.45,-d*.11));
    const bowl=new THREE.Mesh(new THREE.SphereGeometry(.5,48,30),cer);bowl.scale.set(w*.77,h*.24,panD*.58);bowl.position.set(0,seatY-.09,d*.085);bowl.castShadow=true;bowl.receiveShadow=true;g.add(bowl);
    const rearDeck=roundedBox(w*.70,.11,d*.34,.070,cer,0,seatY-.045,-d*.12);g.add(rearDeck);
    // Open bowl recess + separate seat ring and lid give a much stronger toilet silhouette.
    const recess=new THREE.Mesh(new THREE.SphereGeometry(.5,40,24),inn);recess.scale.set(w*.54,.085,d*.36);recess.position.set(0,seatY+.006,d*.10);g.add(recess);
    const seat=roundedRing(w*.69,d*.46,.040,.022,inn,.13);seat.position.set(0,seatY+.045,d*.085);g.add(seat);
    const lid=roundedBox(w*.67,.018,d*.43,.12,cer,0,seatY+.080,d*.070,false);lid.rotation.x=-.025;g.add(lid);
    if(family==="wall-hung"){const shadow=roundedBox(w*.50,.030,d*.32,.05,new THREE.MeshStandardMaterial({color:0x777,transparent:true,opacity:.12}),0,z+.035,-d*.05,false);g.add(shadow)}
    if(detailAtLeast(s,"presentation")){const hingeMat=metal;[-.12,.12].forEach(x=>{const hng=cylinder(.009,.035,hingeMat,"x",18);hng.position.set(x,seatY+.083,-d*.115);g.add(hng)})}
    return setItemId(g,i.id);
  }

  function buildMilanVanity(i,s,prod){
    const g=groupForItem(i),w=mm(i.w||800),d=mm(i.h||450),h=mm(i.height||589),z=mm(i.z||300),wood=furnitureMaterial(prod,s,mats.autumnOak),bodyH=Math.max(.18,h-.045),frontZ=d/2+.010,metal=accessoryMaterial(i,s,mats.brushedBrass,prod);g.add(roundedBox(w,bodyH,d,.015,wood,0,z+bodyH/2,0));const gap=.012,panelH=(bodyH-gap)/2;for(let row=0;row<2;row++){const cy=z+bodyH-(row+.5)*panelH-row*gap;g.add(meshBox(w*.965,panelH-.010,.016,wood,0,cy,frontZ,false));const count=Math.max(22,Math.round(w/(detailAtLeast(s,"presentation")?.017:.022)));for(let n=0;n<count;n++){const px=-w*.46+n*(w*.92/Math.max(1,count-1));g.add(roundedBox(.0065,panelH*.88,.010,.003,mats.oakDark,px,cy,frontZ+.012,false))}g.add(meshBox(w*.56,.010,.014,metal,0,cy+panelH*.31,frontZ+.028,false))}const surfaceY=addIntegratedBasinTop(g,[0],w+.006,d+.010,z+bodyH,.030,Math.min(.46,w*.59),d*.54,s,metal,.028);if(prod?.tapIncluded===true)addMonoblocTap(g,0,surfaceY-.004,-d*.27,metal,.90);return setItemId(g,i.id)
  }

  function buildArezzoVanity(i,s,prod){
    const g=groupForItem(i),w=mm(i.w||900),d=mm(i.h||460),h=mm(i.height||450),z=mm(i.z||400),wood=furnitureMaterial(prod,s,mats.oak),bodyH=Math.max(.18,h-.040),bodyD=d*.94,frontSkin=.028,frontZ=bodyD/2+frontSkin/2+.006,metal=accessoryMaterial(i,s,mats.brushedBrass,prod);g.add(roundedBox(w,bodyH,bodyD,.085,wood,0,z+bodyH/2,0));g.add(roundedBox(w*.985,bodyH*.97,frontSkin,.085,wood,0,z+bodyH/2,frontZ,false));const slatCount=Math.max(16,Math.round(w/(detailAtLeast(s,"presentation")?.030:.040))),span=w*.86;for(let n=0;n<slatCount;n++){const px=-span/2+n*(span/Math.max(1,slatCount-1));g.add(roundedBox(.013,bodyH*.84,.016,.004,mats.autumnOak,px,z+bodyH/2,frontZ+.018,false))}const seamY=z+bodyH*.56;g.add(meshBox(w*.72,.010,.018,mats.oakDark,0,seamY,frontZ+.026,false));const handle=cylinder(.005,Math.max(.18,w*.30),metal,"x",18);handle.position.set(0,seamY-.028,frontZ+.028);g.add(handle);const surfaceY=addIntegratedBasinTop(g,[0],w+.008,d+.008,z+bodyH,.031,Math.min(.47,w*.55),d*.52,s,metal,.075);if(prod?.tapIncluded===true)addMonoblocTap(g,0,surfaceY-.004,-d*.27,metal,.90);g.add(meshBox(w*.88,.016,bodyD*.82,mats.oakDark,0,z+.024,0,false));return setItemId(g,i.id)
  }

  function buildCreteC82754Vanity(i,s,prod){
    const g=groupForItem(i),w=mm(i.w||812),d=mm(i.h||464),h=mm(i.height||849),z=mm(i.z||0),wood=furnitureMaterial(prod,s,mats.apexOak),dark=finishPalette(s).furniture==="product"?mats.apexOakDark:mats.oakDark,bodyH=Math.max(.35,h-.040),frontSkin=.026,frontZ=d/2+frontSkin/2+.004,metal=accessoryMaterial(i,s,mats.chrome,prod);g.add(roundedBox(w,bodyH,d,.018,wood,0,z+bodyH/2,0));const gap=.008,panelH=(bodyH-gap)/2,slatSpan=w*.91,slatCount=Math.max(24,Math.round(w/(detailAtLeast(s,"presentation")?.021:.027)));for(let row=0;row<2;row++){const cy=z+bodyH-(row+.5)*panelH-row*gap;g.add(roundedBox(w*.982,panelH-.006,frontSkin,.012,wood,0,cy,frontZ,false));for(let n=0;n<slatCount;n++){const px=-slatSpan/2+n*(slatSpan/Math.max(1,slatCount-1));g.add(roundedBox(.008,panelH*.94,.012,.003,dark,px,cy,frontZ+.019,false))}const pullW=Math.min(.22,w*.30),pullY=cy+panelH*.40;g.add(roundedBox(pullW,.030,.017,.013,dark,0,pullY,frontZ+.027,false))}const surfaceY=addIntegratedBasinTop(g,[0],w+.006,d+.008,z+bodyH,.030,w*.70,d*.54,s,metal,.020);if(prod?.tapIncluded!==false){const tapHole=cylinder(.009,.006,metal,"y",20);tapHole.position.set(0,surfaceY+.002,-d*.32);g.add(tapHole)}g.add(meshBox(w*.94,.010,d*.88,dark,0,z+.006,0,false));return setItemId(g,i.id)
  }

  function roundedRectPathAt(w,h,r,cx=0,cy=0){const x=cx-w/2,y=cy-h/2,rr=Math.min(Math.abs(r),w/2,h/2),p=new THREE.Path();p.moveTo(x+rr,y);p.lineTo(x+w-rr,y);p.quadraticCurveTo(x+w,y,x+w,y+rr);p.lineTo(x+w,y+h-rr);p.quadraticCurveTo(x+w,y+h,x+w-rr,y+h);p.lineTo(x+rr,y+h);p.quadraticCurveTo(x,y+h,x,y+h-rr);p.lineTo(x,y+rr);p.quadraticCurveTo(x,y,x+rr,y);return p}
  function addMonoblocTap(g,x,y,z,mat,scale=1){const stem=cylinder(.0105*scale,.125*scale,mat,"y",detailAtLeast(api.getState(),"presentation")?28:20);stem.position.set(x,y+.062*scale,z);g.add(stem);const spout=tubePath([new THREE.Vector3(x,y+.100*scale,z),new THREE.Vector3(x,y+.155*scale,z+.005),new THREE.Vector3(x,y+.160*scale,z+.065*scale),new THREE.Vector3(x,y+.135*scale,z+.120*scale)],.010*scale,mat,detailAtLeast(api.getState(),"presentation")?34:24);g.add(spout);const aer=cylinder(.011*scale,.024*scale,mat,"z",18);aer.position.set(x,y+.135*scale,z+.132*scale);g.add(aer);const lever=meshBox(.014*scale,.010*scale,.058*scale,mat,x+.021*scale,y+.116*scale,z-.008,false);lever.rotation.x=-.08;g.add(lever)}
  function addIntegratedBasinTop(g,centres,deckW,deckD,topY,deckH,bowlW,bowlD,s,metal,corner=.055){
    const cer=sanitaryMaterial(s,false),inn=sanitaryMaterial(s,true),shape=roundedRectShape(deckW,deckD,corner);centres.forEach(cx=>shape.holes.push(roundedRectPathAt(bowlW,bowlD,Math.min(.055,bowlD*.20),cx,.018)));
    const geo=new THREE.ExtrudeGeometry(shape,{depth:deckH,steps:1,bevelEnabled:true,bevelSegments:detailAtLeast(s,"presentation")?4:2,bevelSize:.0045,bevelThickness:.004});geo.rotateX(-Math.PI/2);const deck=new THREE.Mesh(geo,cer);deck.position.y=topY;deck.castShadow=true;deck.receiveShadow=true;g.add(deck);
    const surfaceY=topY+deckH,bowlDepth=Math.max(.050,Math.min(.085,bowlD*.25));
    centres.forEach(cx=>{const floorY=topY-bowlDepth*.68;g.add(roundedFootprintBox(bowlW*.76,.010,bowlD*.58,Math.min(.045,bowlD*.18),inn,cx,floorY,.028,false));const wallH=Math.max(.045,surfaceY-floorY),midY=(surfaceY+floorY)/2,front=roundedBox(bowlW*.82,wallH,.018,.010,inn,cx,midY,bowlD*.31,false),back=roundedBox(bowlW*.82,wallH,.018,.010,inn,cx,midY,-bowlD*.29,false);front.rotation.x=.34;back.rotation.x=-.34;g.add(front,back);const left=roundedBox(.018,wallH,bowlD*.55,.010,inn,cx-bowlW*.40,midY,.015,false),right=roundedBox(.018,wallH,bowlD*.55,.010,inn,cx+bowlW*.40,midY,.015,false);left.rotation.z=-.27;right.rotation.z=.27;g.add(left,right);const drain=cylinder(.015,.006,metal,"y",26);drain.position.set(cx,floorY+.012,.030);g.add(drain);const overflow=cylinder(.008,.005,metal,"z",18);overflow.position.set(cx,topY-bowlDepth*.05,-bowlD*.29);g.add(overflow)});return surfaceY
  }
  function addVesselBasin(g,cx,baseY,cz,w,d,h,s,metal){const cer=sanitaryMaterial(s,false),inn=sanitaryMaterial(s,true),outer=roundedRectShape(w,d,Math.min(.07,d*.24)),hole=roundedRectPathAt(w-.050,d-.050,Math.min(.055,d*.20),0,0);outer.holes.push(hole);const geo=new THREE.ExtrudeGeometry(outer,{depth:h,steps:1,bevelEnabled:true,bevelSegments:3,bevelSize:.006,bevelThickness:.005});geo.rotateX(-Math.PI/2);const shell=new THREE.Mesh(geo,cer);shell.position.set(cx,baseY,cz);shell.castShadow=true;g.add(shell);g.add(roundedFootprintBox(w*.72,.010,d*.62,.045,inn,cx,baseY+h*.12,cz,false));const drain=cylinder(.014,.006,metal,"y",24);drain.position.set(cx,baseY+h*.14,cz);g.add(drain)}

  function buildOdesaArc1200Vanity(i,s,prod){
    const g=groupForItem(i),w=mm(i.w||1200),d=mm(i.h||424),h=mm(i.height||855),z=mm(i.z||0);
    const wood=furnitureMaterial(prod,s,mats.californiaWalnut),paletteFurniture=String(finishPalette(s).furniture||"product"),dark=paletteFurniture==="product"?mats.californiaWalnutDark:(paletteFurniture==="white"?mats.ceramicInner:mats.oakDark),groove=paletteFurniture==="product"?mats.californiaWalnutGroove:(paletteFurniture==="white"?new THREE.MeshStandardMaterial({color:0xd6d5d1,roughness:.90}):mats.oakDark);
    const topH=Math.max(.024,Math.min(.034,h*.036)),plinthH=Math.max(.042,Math.min(.060,h*.067)),bodyH=Math.max(.42,h-topH-plinthH),arcR=Math.max(.052,Math.min(.082,d*.19)),bodyY=z+plinthH;
    // Rounded carcass and inset plinth.
    g.add(roundedFootprintBox(w,bodyH,d,arcR,wood,0,bodyY,0));g.add(roundedFootprintBox(w*.925,plinthH,d*.78,Math.max(.018,arcR*.46),dark,0,z,0,false));
    const frontZ=d/2+.010,centreGap=Math.max(.014,Math.min(.020,w*.015)),rowGap=.012,sideMargin=.010,bankW=(w-centreGap)/2,panelW=bankW-sideMargin*2,panelH=(bodyH-rowGap)/2,faceDepth=.015,recessMat=dark;
    g.add(roundedBox(w-arcR*1.25,bodyH*.965,.010,.010,recessMat,0,bodyY+bodyH/2,frontZ-.006,false));
    for(let col=0;col<2;col++){const cx=(col===0?-1:1)*(centreGap/2+bankW/2);for(let row=0;row<2;row++){const cy=bodyY+bodyH-(row+.5)*panelH-row*rowGap;g.add(roundedBox(panelW,panelH-.010,faceDepth,.010,wood,cx,cy,frontZ,false));const usable=panelW*.94,ribStep=detailAtLeast(s,"presentation")?.016:.021,count=Math.max(22,Math.floor(usable/ribStep));for(let n=0;n<=count;n++){const px=cx-usable/2+n*(usable/Math.max(1,count));g.add(roundedBox(.0032,panelH*.905,.0055,.0012,groove,px,cy,frontZ+.0105,false))}const pullY=cy+panelH*.426;g.add(roundedBox(panelW*.86,.011,.008,.004,dark,cx,pullY,frontZ+.0135,false));}}
    g.add(meshBox(centreGap*.62,bodyH*.968,.012,recessMat,0,bodyY+bodyH/2,frontZ+.010,false));const breakY=bodyY+bodyH*.5;for(const cx of [-(centreGap/2+bankW/2),(centreGap/2+bankW/2)])g.add(meshBox(panelW*.985,rowGap*.62,.012,recessMat,cx,breakY,frontZ+.011,false));
    if(detailAtLeast(s,"detailed")){const curveCount=detailAtLeast(s,"presentation")?14:9,cz=d/2-arcR,cxEdge=w/2-arcR;for(let row=0;row<2;row++){const cy=bodyY+bodyH-(row+.5)*panelH-row*rowGap;for(let n=1;n<=curveCount;n++){const a=(Math.PI/2)*(n/(curveCount+1)),zz=cz+arcR*Math.cos(a),xr=cxEdge+arcR*Math.sin(a),xl=-cxEdge-arcR*Math.sin(a);const rr=roundedBox(.0034,panelH*.90,.0055,.0012,groove,xr,cy,zz+.008,false);rr.rotation.y=a;g.add(rr);const rl=roundedBox(.0034,panelH*.90,.0055,.0012,groove,xl,cy,zz+.008,false);rl.rotation.y=-a;g.add(rl)}}}
    const topStyle=String(i.vanityTopStyle||i.config?.topStyle||prod?.defaultTopStyle||"countertop").toLowerCase(),metal=accessoryMaterial(i,s,mats.brushedBrass,prod),topY=z+plinthH+bodyH;
    if(topStyle==="double-basin"){
      const deckH=Math.max(.026,Math.min(.034,h*.040)),deckW=w+.018,deckD=d+.026,bowlW=Math.max(.29,w*.32),bowlD=Math.max(.205,d*.50),centres=[-w*.245,w*.245];const surfaceY=addIntegratedBasinTop(g,centres,deckW,deckD,topY,deckH,bowlW,bowlD,s,metal,arcR+.016);
      // Low rear splash lip and two deck-mounted taps, as in the reference product photography.
      g.add(roundedBox(deckW*.96,.020,.022,.007,sanitaryMaterial(s,false),0,surfaceY-.010,-deckD/2+.014,false));centres.forEach(cx=>addMonoblocTap(g,cx,surfaceY-.002,-deckD*.245,metal,.92));
    }else if(topStyle==="double-vessel"){
      g.add(roundedFootprintBox(w+.010,topH,d+.010,arcR+.005,wood,0,topY,0));const y=topY+topH;[-w*.245,w*.245].forEach(cx=>{addVesselBasin(g,cx,y,.018,w*.31,d*.55,.115,s,metal);addMonoblocTap(g,cx,y+.005,-d*.31,metal,.92)});
    }else{
      g.add(roundedFootprintBox(w+.010,topH,d+.010,arcR+.005,wood,0,topY,0));g.add(roundedBox(w-arcR*2.10,topH*.48,.007,.003,groove,0,topY+topH*.54,d/2+.010,false));
    }
    return setItemId(g,i.id)
  }
  ...
  function buildVanity(i,s){
    ...
    const g=groupForItem(i),w=mm(i.w||800),d=mm(i.h||450),h=mm(i.height||850),z=mm(i.z||0),wood=furnitureMaterial(prod,s,prod?.finish?.toLowerCase().includes("oak")?mats.oak:mats.timber),bodyH=Math.max(.18,h-.060),metal=accessoryMaterial(i,s,mats.brushedBrass,prod),frontZ=d/2+.011;
    g.add(roundedBox(w,bodyH,d,.018,wood,0,z+bodyH/2,0));const gap=.010,panelH=(bodyH-gap)/2;for(let row=0;row<2;row++){const cy=z+bodyH-(row+.5)*panelH-row*gap;g.add(roundedBox(w*.96,panelH-.010,.018,.012,wood,0,cy,frontZ,false));if(detailAtLeast(s,"detailed")){const handle=cylinder(.0065,w*.26,metal,"x",20);handle.position.set(0,cy+panelH*.30,frontZ+.020);g.add(handle)}}
    const basinCfg=String(i.v3?.basin||"auto").toLowerCase(),autoDouble=w>=1100,mode=basinCfg==="auto"?(autoDouble?"double-integrated":"single-integrated"):basinCfg,centres=mode.startsWith("double")?[-w*.245,w*.245]:[0],topY=z+bodyH;
    if(mode.includes("vessel")){
      const topMat=furnitureMaterial(prod,s,wood);g.add(roundedFootprintBox(w+.010,.032,d+.012,.020,topMat,0,topY,0));centres.forEach(cx=>{addVesselBasin(g,cx,topY+.032,.018,Math.min(w*.36,.38),d*.56,.115,s,metal);addMonoblocTap(g,cx,topY+.035,-d*.31,metal,.90)});
    }else{
      const deckH=.030,bowlW=Math.min(mode.startsWith("double")?w*.34:w*.58,.43),bowlD=d*.54,surfaceY=addIntegratedBasinTop(g,centres,w+.010,d+.012,topY,deckH,bowlW,bowlD,s,metal,.025);centres.forEach(cx=>addMonoblocTap(g,cx,surfaceY-.004,-d*.265,metal,.88));
    }
    return setItemId(g,i.id)
  }
  ...
  function buildGlassItem(i,s){
    const g=groupForItem(i),w=mm(i.w||900),d=Math.max(.006,mm(i.glassThickness||i.h||10)),h=mm(i.height||1000),z=mm(i.z??1100),palette=finishPalette(s),style=i.v3?.glassOverride?(i.glassStyle||"fluted"):(palette.glass||i.glassStyle||"fluted"),privacy=i.glassPrivacy||"light";
    if(style==="none")return setItemId(g,i.id);
    let baseMat=(style==="fluted"?mats.flutedGlass:mats.glass).clone();baseMat.userData.bpOwned=true;baseMat.thickness=Math.max(.008,d);baseMat.ior=1.47;
    if(style==="frosted"){baseMat=mats.glass.clone();baseMat.userData.bpOwned=true;baseMat.transmission=.48;baseMat.opacity=privacy==="medium"?.55:.46;baseMat.roughness=.52}
    else if(privacy==="clear"){baseMat.opacity=style==="fluted"?.25:.19}
    else if(privacy==="medium"){baseMat.opacity=style==="fluted"?.43:.37;baseMat.roughness=Math.max(baseMat.roughness,.30)}
    const base=meshBox(w,h,d,baseMat,0,z+h/2,0,false);base.renderOrder=3;g.add(base);
    if(style==="fluted"){
      const spacing=detailAtLeast(s,"presentation")?.024:.030,count=Math.max(4,Math.min(56,Math.floor(w/spacing))),actual=w/count,ribMat=new THREE.MeshPhysicalMaterial({color:0xc7dde0,transmission:.48,transparent:true,opacity:privacy==="medium"?.38:.29,roughness:.36,metalness:0,side:THREE.DoubleSide,depthWrite:false});ribMat.userData.bpOwned=true;
      for(let n=1;n<count;n++){const x=-w/2+n*actual,rib=meshBox(Math.min(.0065,actual*.24),h*.987,.0035,ribMat,x,z+h/2,d/2+.0018,false);rib.renderOrder=4;g.add(rib)}
    }
    const trim=trimFinishMaterial(i,s),edge=.0055,trimDepth=Math.max(d+.004,.010);
    // Slim U-channels on wall/ceiling-facing edges and a low floor/deck channel.
    g.add(meshBox(w,edge,trimDepth,trim,0,z+h-edge/2,0,false));g.add(meshBox(w,edge*.85,trimDepth,trim,0,z+edge/2,0,false));g.add(meshBox(edge,h,trimDepth,trim,-w/2+edge/2,z+h/2,0,false));g.add(meshBox(edge,h,trimDepth,trim,w/2-edge/2,z+h/2,0,false));
    if(detailAtLeast(s,"detailed")){[-w*.28,w*.28].forEach(x=>{const clamp=roundedBox(.034,.020,.016,.004,trim,x,z+.020,d/2+.008,false);g.add(clamp)});}
    if(detailAtLeast(s,"presentation")){const edgeGlass=new THREE.MeshStandardMaterial({color:0x8fbcc0,roughness:.22,transparent:true,opacity:.40});g.add(meshBox(w,.003,d+.001,edgeGlass,0,z+h+.001,0,false));}
    return setItemId(g,i.id);
  }
  ...
  function buildNiche(i,s) {
    const z0=mm(i.z||1000),hh=mm(i.height||350),depth=Math.max(.04,mm(i.nicheDepth||i.h||90));
    if(i.mountHost){
      const g=mountedGroup(i,s),width=Math.max(.10,mm(i.w||400)),frame=.018,reveal=mats.ceramicMattInner,shadow=new THREE.MeshStandardMaterial({color:0x69635b,roughness:.96});
      // A dark opening at the wall face plus recessed back and four returns makes the niche read as a real cavity without expensive CSG wall cutting.
      g.add(roundedBox(width+.018,hh+.018,.008,.010,shadow,0,z0+hh/2,-.005,false));
      g.add(roundedBox(Math.max(.06,width-frame*2),Math.max(.06,hh-frame*2),.012,.008,mats.niche,0,z0+hh/2,-depth,false));
      g.add(meshBox(width,frame,depth,reveal,0,z0+frame/2,-depth/2,false));
      g.add(meshBox(width,frame,depth,reveal,0,z0+hh-frame/2,-depth/2,false));
      g.add(meshBox(frame,Math.max(.04,hh-frame*2),depth,reveal,-width/2+frame/2,z0+hh/2,-depth/2,false));
      g.add(meshBox(frame,Math.max(.04,hh-frame*2),depth,reveal,width/2-frame/2,z0+hh/2,-depth/2,false));
      return setItemId(g,i.id);
    }
  ...
  function pipePointZ(route,index){
    ...
    const f=total>0?run/total:((index-aIdx)/Math.max(1,bIdx-aIdx));return aZ+(bZ-aZ)*THREE.MathUtils.clamp(f,0,1);
  }
  function plumbingFocusActive(){return api.getState()?.ui?.plumbingFocus===true}
  function buildPipeRoutes(s){
    const colors={hot:0xd84a3d,cold:0x2878d8,mixed:0x8a54d6,waste:0x159a7f,soil:0x363a3a},focus=plumbingFocusActive(),selected=window.BPPipes?.getSelectedRouteId?.(),routes=[...(s.pipeRoutes||[])];
    const draft3d=window.BPPipes?.getDraft3D?.();if(draft3d)routes.push({...draft3d,__draft3d:true});
    routes.filter(r=>r.visible!==false&&(r.points||[]).length>0).forEach(route=>{
      const pts=(route.points||[]).map((p,i)=>new THREE.Vector3(mm(p.x),mm(pipePointZ(route,i)),mm(p.y))),isSel=route.id===selected||route.__draft3d;
      const radius=Math.max(.006,Math.min(.055,mm(route.diameter||15)/2))*(focus?1.16:1)*(isSel?1.24:1);
      const col=colors[route.type]||0x777777,mat=new THREE.MeshStandardMaterial({color:col,roughness:focus?.24:.42,metalness:.04,emissive:(focus||isSel)?col:0x000000,emissiveIntensity:isSel?.35:(focus?.20:0)});mat.userData.bpOwned=true;
      for(let i=1;i<pts.length;i++){const a=pts[i-1],b=pts[i],dir=b.clone().sub(a),len=dir.length();if(len<.001)continue;const geo=new THREE.CylinderGeometry(radius,radius,len,(focus||isSel)?18:12),seg=new THREE.Mesh(geo,mat);seg.position.copy(a).add(b).multiplyScalar(.5);seg.quaternion.setFromUnitVectors(new THREE.Vector3(0,1,0),dir.normalize());seg.castShadow=!focus;seg.receiveShadow=true;seg.userData.pipeRouteId=route.id;seg.userData.pipeDraft=!!route.__draft3d;pipeRoot.add(seg);}
      pts.forEach((pt,idx)=>{const joint=new THREE.Mesh(new THREE.SphereGeometry(radius*(isSel?1.45:1.18),(focus||isSel)?18:14,(focus||isSel)?14:10),mat);joint.position.copy(pt);joint.userData.pipeRouteId=route.id;joint.userData.pipeNodeIndex=idx;joint.userData.pipeDraft=!!route.__draft3d;pipeRoot.add(joint)});
    });
  }
  function ghostGroup(group,opacity){
    group.traverse(o=>{if(!o?.isMesh||!o.material)return;const make=base=>{const m=base.clone();m.userData={...(m.userData||{}),bpOwned:true};m.transparent=true;m.opacity=Math.min(Number.isFinite(Number(base.opacity))?Number(base.opacity):1,opacity);m.depthWrite=false;if('transmission' in m&&Number(m.transmission)>0)m.transmission=Math.min(Number(m.transmission),.35);return m};o.material=Array.isArray(o.material)?o.material.map(make):make(o.material);o.castShadow=false;});
  }
  function applyPlumbingFocusMaterials(s){
    const focus=s?.ui?.plumbingFocus===true;
    legacy.closest('.view3dShell')?.classList.toggle('plumbingFocus3d',focus);
    if(plumbingFocusBtn){plumbingFocusBtn.classList.toggle('active3d',focus);plumbingFocusBtn.setAttribute('aria-pressed',focus?'true':'false');plumbingFocusBtn.textContent=focus?'Plumbing focus: On':'Plumbing focus';}
    if(!focus)return;
    ghostGroup(root,.10);ghostGroup(wallRoot,.11);ghostGroup(surfaceRoot,.08);ghostGroup(itemRoot,.24);ghostGroup(fixedRoot,.18);
  }
  function setPlumbingFocus(on){const s=api.getState();s.ui=s.ui||{};s.ui.plumbingFocus=!!on;api.persist?.();rebuildScene(true);updateWallVisibility();status.textContent=on?'Plumbing focus · fixtures ghosted · joists solid · pipes prioritised':'Visual 3D · renderer V3.0.5 · tap a fixture or pipe to edit';}
  ...
  function rebuildScene(force=false) {
    ...
    const sig=JSON.stringify({
      ...
      heating:s.heating||{},
      pipeRoutes:(s.pipeRoutes||[]).map(r=>({id:r.id,type:r.type,diameter:r.diameter,zStart:r.zStart,zEnd:r.zEnd,visible:r.visible,points:r.points})),
      finishPalette:s.project?.finishPalette||{},
      globalFinish:s.project?.globalFixtureFinish||"use-item",
      fidelity:s.ui?.fidelity||"detailed",
      wallMode:s.ui?.wallMode||"auto",
      plumbingFocus:s.ui?.plumbingFocus===true
    });
    if(!force && sig===signature) return;
    signature=sig; room=s.room;
    clearGroup(root);clearGroup(wallRoot);clearGroup(surfaceRoot);clearGroup(structureRoot);clearGroup(itemRoot);clearGroup(fixedRoot);clearGroup(pipeRoot);
    itemMeshes=[];
    buildFloor(s);buildWalls(s);buildSurfaceZones(s);buildStructure(s);buildPipeRoutes(s);
    (s.items||[]).filter(i=>i.visible!==false).forEach(i=>{
      const g=buildItem(i,s);
      itemRoot.add(g);
      itemMeshes.push(g);
    });
    if(!sceneInitialised){target.set(mm(s.room.width)/2,.92,mm(s.room.depth)/2);sceneInitialised=true;}
    applyFidelity(s.ui?.fidelity||"detailed",false);
    applyPlumbingFocusMaterials(s);
    updateCamera();
  }
  ...
  function currentWallMode(){return String(api.getState()?.ui?.wallMode||wallModeSelect?.value||"auto").toLowerCase()}
  function applyFidelity(mode,refresh=true){
    mode=String(mode||"detailed").toLowerCase();const s=api.getState();if(s?.ui)s.ui.fidelity=mode;if(fidelityModeSelect)fidelityModeSelect.value=mode;
    if(mode==="draft"){renderer.setPixelRatio(Math.min(1.05,window.devicePixelRatio||1));renderer.shadowMap.enabled=false;renderer.toneMappingExposure=1.10;key.shadow.mapSize.set(512,512);legacy.parentNode?.classList.remove("presentation3d");}
    else if(mode==="presentation"){renderer.setPixelRatio(Math.min(2.0,window.devicePixelRatio||1));renderer.shadowMap.enabled=true;renderer.toneMappingExposure=1.20;key.shadow.mapSize.set(2048,2048);legacy.parentNode?.classList.add("presentation3d");}
    else{renderer.setPixelRatio(Math.min(1.55,window.devicePixelRatio||1));renderer.shadowMap.enabled=true;renderer.toneMappingExposure=1.16;key.shadow.mapSize.set(1536,1536);legacy.parentNode?.classList.remove("presentation3d");}
    if(refresh){resize();rebuildScene(true)}
  }
  function setWallMode(mode){mode=["auto","all","none"].includes(mode)?mode:"auto";const s=api.getState();s.ui=s.ui||{};s.ui.wallMode=mode;if(wallModeSelect)wallModeSelect.value=mode;if(wallsToggle)wallsToggle.checked=mode!=="none";updateWallVisibility();}
  function updateWallVisibility() {
    if(!room)return;
    const W=mm(room.width),D=mm(room.depth),mode=currentWallMode(),show=mode!=="none",xray=floorXrayToggle?.checked===true,plumbingFocus=plumbingFocusActive();
    root.visible=plumbingFocus||!xray;structureRoot.visible=plumbingFocus||xray;itemRoot.visible=true;fixedRoot.visible=true;pipeRoot.visible=true;measureRoot.visible=true;
    structureRoot.children.forEach(m=>{const kind=m.userData?.structureKind;if(plumbingFocus){m.visible=kind!=="deck"&&kind!=="heating";}else if(kind==="deck"||kind==="heating")m.visible=!xray;else m.visible=xray;});
    Object.values(wallSets).flat().forEach(m=>m.visible=show);Object.entries(surfaceSets).forEach(([side,list])=>list.forEach(m=>m.visible=(side==="floor")?!xray:show));
    if(!show||mode==="all")return;
    // Auto cutaway: remove every boundary wall that sits between the current camera and the room.
    const margin=.035;
    ...
    // When orbiting from a high interior/near-boundary position, also open the closest wall if it dominates the view.
    if(camera.position.x>=0&&camera.position.x<=W&&camera.position.z>=0&&camera.position.z<=D&&!walkMode){
      const distances=[{k:"left",v:camera.position.x},{k:"right",v:W-camera.position.x},{k:"window",v:camera.position.z},{k:"opposite",v:D-camera.position.z}].sort((a,b)=>a.v-b.v);if(distances[0].v<.18){wallSets[distances[0].k].forEach(m=>m.visible=false);surfaceSets[distances[0].k].forEach(m=>m.visible=false)}
    }
  }
  ...
  function walkBlocked(pos){
    ...
    return (s.items||[]).some(i=>{if(i.visible===false)return false;if(["mirror","rainHead","handset","showerControls","niche","glassPanel","radiator"].includes(i.type))return false;if(i.type==="shower"&&(Number(i.height)||40)<120)return false;const d=itemDims(i),top=(Number(i.z)||0)+(Number(i.height)||0);if(top<250)return false;return px>=i.x-pad&&px<=i.x+d.w+pad&&pz>=i.y-pad&&pz<=i.y+d.d+pad});
  }
  ...
  function itemWorldTarget(item){if(!item)return null;const d=itemDims(item);return new THREE.Vector3(mm((Number(item.x)||0)+d.w/2),mm((Number(item.z)||0)+(Number(item.height)||800)*.48),mm((Number(item.y)||0)+d.d/2))}
  function frameFixture(type){
    const s=api.getState(),W=mm(s.room.width),D=mm(s.room.depth),items=(s.items||[]).filter(i=>i.visible!==false&&i.type===type);if(!items.length)return false;
    const item=items.sort((a,b)=>(Number(b.w)||0)*(Number(b.h)||0)-(Number(a.w)||0)*(Number(a.h)||0))[0],t=itemWorldTarget(item),centre=new THREE.Vector3(W/2,1.0,D/2),dir=centre.clone().sub(t);dir.y=0;if(dir.lengthSq()<.02)dir.set(1,0,1);dir.normalize();const distance=type==="bath"?2.15:type==="shower"?2.0:1.75,pos=t.clone().addScaledVector(dir,distance);pos.y=Math.max(1.25,t.y+.55);setFromPosition(pos,t);return true;
  }
  function preset(name) {
    if(walkMode)setWalkMode(false,false);const s=api.getState(),W=mm(s.room.width),D=mm(s.room.depth),H=mm(s.room.ceiling);
    if(["vanity","bath","shower","wc"].includes(name)){if(frameFixture(name))return;}
    if(name==="door"){const dz=mm(s.room.door.before+s.room.door.width/2);setFromPosition(new THREE.Vector3(W+2.0,1.55,dz),new THREE.Vector3(W*.45,.95,D*.50));}
    else if(name==="window")setFromPosition(new THREE.Vector3(W*.50,1.55,-2.05),new THREE.Vector3(W*.50,.95,D*.52));
    else if(name==="top")setFromPosition(new THREE.Vector3(W*.50,5.3,D*.50+.01),new THREE.Vector3(W*.50,0,D*.50));
    else if(name==="corner")setFromPosition(new THREE.Vector3(W+1.55,2.35,D+1.55),new THREE.Vector3(W*.48,.85,D*.50));
    else if(name==="eye"){const dz=mm(s.room.door.before+s.room.door.width/2);setFromPosition(new THREE.Vector3(W-.16,1.62,THREE.MathUtils.clamp(dz,.20,D-.20)),new THREE.Vector3(W*.42,1.20,D*.50));}
    else if(name==="saved"&&s.ui?.savedCamera){const c=s.ui.savedCamera;setFromPosition(new THREE.Vector3(c.position.x,c.position.y,c.position.z),new THREE.Vector3(c.target.x,c.target.y,c.target.z));}
    else if(name==="underfloor"){const depth=mm(Math.max(120,Number(s.structure?.depth)||195));setFromPosition(new THREE.Vector3(W*.50,-Math.max(1.05,depth*3.8),D+1.55),new THREE.Vector3(W*.50,-depth*.45,D*.50));}
    else setFromPosition(new THREE.Vector3(W+1.75,3.05,D+1.85),new THREE.Vector3(W*.48,.90,D*.50));
  }
  function saveCurrentCamera(){const s=api.getState();s.ui=s.ui||{};s.ui.savedCamera={position:{x:camera.position.x,y:camera.position.y,z:camera.position.z},target:{x:target.x,y:target.y,z:target.z},savedAt:new Date().toISOString()};api.persist?.();status.textContent="Camera view saved";setTimeout(()=>status.textContent="Visual 3D · renderer V3.0.5 · tap a fixture or pipe to edit",1400);}
  ...
  function clear3DMeasure(){while(measureRoot.children.length){const o=measureRoot.children.pop();o.geometry?.dispose?.();if(o.material&&!sharedMaterials.has(o.material))o.material.dispose?.()}measurePoints=[];}
  function measureAt(clientX,clientY){rayFromClient(clientX,clientY);const roots=[...itemRoot.children,...surfaceRoot.children,...wallRoot.children,...root.children].filter(o=>o.visible!==false),hits=raycaster.intersectObjects(roots,true).filter(h=>h.object.visible!==false);let p=hits[0]?.point?.clone();if(!p){const floorPlane=new THREE.Plane(new THREE.Vector3(0,1,0),0),tmp=new THREE.Vector3();if(raycaster.ray.intersectPlane(floorPlane,tmp))p=tmp.clone()}if(!p)return;measurePoints.push(p);const marker=new THREE.Mesh(new THREE.SphereGeometry(.018,18,12),mats.brushedBrass);marker.position.copy(p);marker.renderOrder=10;measureRoot.add(marker);if(measurePoints.length===2){const [a,b]=measurePoints,geo=new THREE.BufferGeometry().setFromPoints([a,b]),line=new THREE.Line(geo,new THREE.LineBasicMaterial({color:0x2d302d}));line.renderOrder=10;measureRoot.add(line);const dist=a.distanceTo(b)*1000;status.textContent=`3D measurement · ${Math.round(dist)} mm`;measureMode3D=false;measure3DBtn?.classList.remove("active3d");measure3DBtn?.setAttribute("aria-pressed","false");}}
  function toggleMeasure3D(){measureMode3D=!measureMode3D;clear3DMeasure();measure3DBtn?.classList.toggle("active3d",measureMode3D);measure3DBtn?.setAttribute("aria-pressed",measureMode3D?"true":"false");status.textContent=measureMode3D?"3D measure · tap first surface":"Visual 3D · renderer V3.0.5 · tap a fixture or pipe to edit";}
  function applySection(mode){sectionMode=sectionMode===mode?null:mode;const s=api.getState(),W=mm(s.room.width),D=mm(s.room.depth);renderer.clippingPlanes=[];if(sectionMode==="x")renderer.clippingPlanes=[new THREE.Plane(new THREE.Vector3(-1,0,0),W*.52)];if(sectionMode==="y")renderer.clippingPlanes=[new THREE.Plane(new THREE.Vector3(0,0,-1),D*.52)];sectionXBtn?.classList.toggle("active3d",sectionMode==="x");sectionYBtn?.classList.toggle("active3d",sectionMode==="y");sectionXBtn?.setAttribute("aria-pressed",sectionMode==="x"?"true":"false");sectionYBtn?.setAttribute("aria-pressed",sectionMode==="y"?"true":"false");status.textContent=sectionMode?`Section ${sectionMode.toUpperCase()} · cut at room centre`:"Visual 3D · renderer V3.0.5 · tap a fixture or pipe to edit";}
  async function toggleFullscreen(){const shell=legacy.closest(".view3dShell");if(!shell)return;try{if(document.fullscreenElement){await document.exitFullscreen()}else if(shell.requestFullscreen){await shell.requestFullscreen()}else throw Error("Fullscreen API unavailable");setTimeout(resize,80)}catch(_){const on=!shell.classList.contains("v3PseudoFullscreen");shell.classList.toggle("v3PseudoFullscreen",on);document.body.classList.toggle("v3FullscreenLock",on);setTimeout(resize,80)}}
  ...
  function replaceItemVisual(id){
    ...
    if(item.visible!==false)itemRoot.add(buildItem(item,state));
  }

  function isWallMountedMove(item){return !!item&&(item.type==="mirror"||item.type==="rainHead"||item.type==="handset"||item.type==="showerControls"||(item.type==="niche"&&item.mountHost))}
  ...
  function startMoveMode(id){
    ...
    if(!item||item.locked||(item.type==="niche"&&!item.mountHost))return;
  ...
  window.BP3DView={startMove:startMoveMode,stopMove:()=>stopMoveMode(true),refresh:()=>rebuildScene(true),setWallMode,setFidelity:(m)=>applyFidelity(m,true),preset,saveCamera:saveCurrentCamera,section:applySection,toggleMeasure:toggleMeasure3D,closeAddMenu:()=>closeAdd3DMenu()};
  document.querySelectorAll(".tab").forEach(tab=>tab.addEventListener("click",()=>{if(tab.dataset.tab!=="threeD"&&moveMode)stopMoveMode(true);if(tab.dataset.tab!=="threeD"&&walkMode)setWalkMode(false,false);if(tab.dataset.tab!=="threeD"&&nichePlaceMode)cancelNiche3DPlace();if(tab.dataset.tab!=="threeD")closeAdd3DMenu();}));

  function closeAdd3DMenu(){add3DMenu?.classList.add("hidden");add3DBtn?.setAttribute("aria-expanded","false");}
  function toggleAdd3DMenu(){const opening=add3DMenu?.classList.contains("hidden");if(opening){if(nichePlaceMode)cancelNiche3DPlace();add3DMenu?.classList.remove("hidden");add3DBtn?.setAttribute("aria-expanded","true");}else closeAdd3DMenu();}
  ...
  function sceneSurfaceHit(clientX,clientY,{nicheOnly=false}={}){rayFromClient(clientX,clientY);const s=api.getState(),candidates=[];if(nicheOnly){itemRoot.children.forEach(g=>{const id=g.userData?.itemId,i=s.items?.find(x=>x.id===id);if(i?.type==="stud"&&g.visible!==false)candidates.push(g)});wallRoot.children.forEach(o=>{if(o.visible!==false)candidates.push(o)});}else{wallRoot.children.forEach(o=>{if(o.visible!==false)candidates.push(o)});structureRoot.children.forEach(o=>{if(o.visible!==false)candidates.push(o)});root.children.forEach(o=>{if(o.visible!==false&&o.isMesh)candidates.push(o)});itemRoot.children.forEach(o=>{if(o.visible!==false)candidates.push(o)});}const hits=raycaster.intersectObjects(candidates,true).filter(h=>h.object.visible!==false&&h.object.isMesh);return hits[0]||null;}
  function snappedPipePointFromHit(hit){if(!hit)return null;const s=api.getState(),p=hit.point.clone(),itemId=ancestorData(hit.object,"itemId"),item=itemId?s.items?.find(x=>x.id===itemId):null,wallSide=ancestorData(hit.object,"wallSide");if(item?.type==="stud"){const sd=itemDims(item),yaw=-THREE.MathUtils.degToRad(Number(item.rotation)||0),center=new THREE.Vector3(mm(item.x+sd.w/2),0,mm(item.y+sd.d/2)),tangent=new THREE.Vector3(Math.cos(yaw),0,-Math.sin(yaw)),delta=p.clone().sub(center),along=delta.dot(tangent);p.x=center.x+tangent.x*along;p.z=center.z+tangent.z*along;return{point:p,label:"stud centre"};}const embed=.020,W=mm(s.room.width),D=mm(s.room.depth);if(wallSide==="left")p.x=embed;else if(wallSide==="right")p.x=W-embed;else if(wallSide==="window")p.z=embed;else if(wallSide==="opposite")p.z=D-embed;return{point:p,label:wallSide?`${wallSide} wall`:item?.name||ancestorData(hit.object,"structureKind")||"surface"};}
  function updateNiche3DOverlay(){niche3DPlaceBar?.classList.toggle("hidden",!nichePlaceMode);add3DBtn?.classList.toggle("active3d",nichePlaceMode);}
  function startNiche3DPlace(){window.BPPipes?.cancelDraw?.();closeAdd3DMenu();if(walkMode)setWalkMode(false,false);if(moveMode)stopMoveMode(true);measureMode3D=false;measure3DBtn?.classList.remove("active3d");nichePlaceMode=true;updateNiche3DOverlay();status.textContent="Place recess · tap a room wall or stud face at the centre of the recess";}
  function cancelNiche3DPlace(){nichePlaceMode=false;updateNiche3DOverlay();status.textContent="Visual 3D · renderer V3.0.5 · tap a fixture or pipe to edit";}
  function placeNiche3DAt(clientX,clientY){
    if(!nichePlaceMode)return false;
    const s=api.getState(),hit=sceneSurfaceHit(clientX,clientY,{nicheOnly:true});
    if(!hit){status.textContent="Place recess · tap a solid room wall or stud face";return true;}
    const p=hit.point,width=Math.max(100,Number(niche3DWidth?.value)||400),height=Math.max(100,Number(niche3DHeight?.value)||300),depth=Math.max(40,Number(niche3DDepth?.value)||90);
    let host=null,along=0,mountFace="shower";
    const hitItemId=ancestorData(hit.object,"itemId"),stud=hitItemId?s.items?.find(x=>x.id===hitItemId&&x.type==="stud"):null;
    if(stud){
      host="stud:"+stud.id;const sd=itemDims(stud),rawW=mm(Number(stud.w)||900),yaw=-THREE.MathUtils.degToRad(Number(stud.rotation)||0),center=new THREE.Vector3(mm(stud.x+sd.w/2),0,mm(stud.y+sd.d/2)),tangent=new THREE.Vector3(Math.cos(yaw),0,-Math.sin(yaw)),delta=p.clone().sub(center);along=delta.dot(tangent)*1000+(Number(stud.w)||900)/2;
      const showerSign=studShowerFaceSign(stud,s),normal=new THREE.Vector3(Math.sin(yaw)*showerSign,0,Math.cos(yaw)*showerSign);mountFace=delta.dot(normal)>=0?"shower":"other";
    }else{
      host=ancestorData(hit.object,"wallSide")||null;if(!host){status.textContent="Place recess · choose the room wall itself rather than a fixture";return true;}along=(host==="left"||host==="right")?p.z*1000:p.x*1000;
    }
    const z=Math.max(0,Math.min((Number(s.room?.ceiling)||2470)-height,p.y*1000-height/2)),created=api.addNiche3D?.({host,along,z,width,height,depth,mountFace});
    if(created){nichePlaceMode=false;updateNiche3DOverlay();rebuildScene(true);status.textContent=`Recess added · ${Math.round(width)} × ${Math.round(height)} × ${Math.round(depth)} mm · select Move in 3D to fine-tune`;}
    return true;
  }
  add3DBtn?.addEventListener("click",e=>{e.stopPropagation();toggleAdd3DMenu();});
  document.querySelectorAll("[data-add3d-pipe]").forEach(b=>b.addEventListener("click",()=>{closeAdd3DMenu();nichePlaceMode=false;updateNiche3DOverlay();window.BPPipes?.startDraw3D?.(b.dataset.add3dPipe);}));
  add3DNicheBtn?.addEventListener("click",startNiche3DPlace);niche3DCancelBtn?.addEventListener("click",cancelNiche3DPlace);
  document.addEventListener("pointerdown",e=>{if(!add3DMenu?.classList.contains("hidden")&&!add3DMenu.contains(e.target)&&e.target!==add3DBtn)closeAdd3DMenu();});

  function selectAt(clientX,clientY) {
    rayFromClient(clientX,clientY);
    const pipeHits=raycaster.intersectObjects(pipeRoot.children,true),pipeHit=pipeHits.find(h=>h.object.userData.pipeRouteId&&!h.object.userData.pipeDraft);
    if(pipeHit){window.BPPipes?.selectRoute?.(pipeHit.object.userData.pipeRouteId);rebuildScene(true);status.textContent="Pipe selected · move it in X/Y/Z, drag it in Plan, or delete it";return;}
    const hits=raycaster.intersectObjects(itemRoot.children,true);
    const hit=hits.find(h=>h.object.userData.itemId);
    if(hit){
      window.BPPipes?.selectRoute?.(null);api.selectItem(hit.object.userData.itemId);
      status.textContent="Selected · edit in the panel below";
      setTimeout(()=>status.textContent="Visual 3D · renderer V3.0.5 · tap a fixture or pipe to edit",1800);
    }
  }
  function addPipe3DAt(clientX,clientY){
    if(!window.BPPipes?.isDrawing3D?.())return false;
    const hit=sceneSurfaceHit(clientX,clientY);if(!hit){status.textContent="3D pipe draw · tap a room, floor, wall, stud, joist or fixture surface";return true;}
    const snap=snappedPipePointFromHit(hit),p=snap?.point||hit.point,added=window.BPPipes.add3DPoint?.({x:p.x*1000,y:p.z*1000,z:p.y*1000});
    if(added){closeAdd3DMenu();rebuildScene(true);const mode=window.BPPipes.get3DMode?.()||"level";status.textContent=`3D pipe draw · point fixed to ${snap?.label||"surface"} · ${mode} next`;}
    return true;
  }
  ...
  function endPointer(e){
    ...
    if(was&&!was.moved&&!moveMode){if(nichePlaceMode)placeNiche3DAt(e.clientX,e.clientY);else if(window.BPPipes?.isDrawing3D?.())addPipe3DAt(e.clientX,e.clientY);else if(measureMode3D)measureAt(e.clientX,e.clientY);else selectAt(e.clientX,e.clientY);}
    if(was)drag=null;
  }
  ...
  function setFloorXray(on, moveCamera=true){
    ...
    status.textContent=on?"Floor removed · fixtures stay visible · joists exposed":(plumbingFocusActive()?"Plumbing focus · fixtures ghosted · joists solid · pipes prioritised":"Visual 3D · renderer V3.0.5 · tap a fixture or pipe to edit");
  }
  ...
  wallModeSelect?.addEventListener("change",e=>{setWallMode(e.target.value);api.persist?.()});
  fidelityModeSelect?.addEventListener("change",e=>{const st=api.getState();st.ui=st.ui||{};st.ui.fidelity=e.target.value;api.persist?.();applyFidelity(e.target.value,true)});
  cameraPresetSelect?.addEventListener("change",e=>{if(e.target.value){preset(e.target.value);e.target.value=""}});
  saveCameraBtn?.addEventListener("click",saveCurrentCamera);
  measure3DBtn?.addEventListener("click",toggleMeasure3D);
  fullScreen3DBtn?.addEventListener("click",toggleFullscreen);document.addEventListener("fullscreenchange",()=>setTimeout(resize,80));
  plumbingFocusBtn?.addEventListener("click",()=>setPlumbingFocus(!plumbingFocusActive()));
  ...
  const initialState=api.getState();if(wallModeSelect)wallModeSelect.value=initialState?.ui?.wallMode||"auto";if(fidelityModeSelect)fidelityModeSelect.value=initialState?.ui?.fidelity||"detailed";setWallMode(initialState?.ui?.wallMode||"auto");applyFidelity(initialState?.ui?.fidelity||"detailed",false);
  rebuildScene(true);
  preset("reset");
  setFloorXray(false,false);
  if(initialState?.ui?.plumbingFocus===true){applyPlumbingFocusMaterials(initialState);updateWallVisibility();status.textContent="Plumbing focus · fixtures ghosted · joists solid · pipes prioritised";}
  syncViewMode();
  animate();
}
